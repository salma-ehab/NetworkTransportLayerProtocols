...
//B sequence number
int seqB;
//waiting variable to check if the positive acknowledgement was recieved before sending the next packet
int waitingFlag;
...
{
  //if the last packet hasn't been positively acknowledged, do nothing
	if (waitingFlag) 
  {
		printf("The last packet hasn't been acknowledged yet\n");
		return;
	}

  //create the packet to be sent
	pktA= createpacket(message);
  //send the packet onto the network
	tolayer3(0, pktA);
  //start the timer
	starttimer(0, TIME_OUT);
  //change the waiting flag from 0 to 1 so as not to send another packet until the last one is positively acknowledged 
  waitingFlag = 1;
...
{ 
   printPacketB("This packet was sent by B:",packet);

   // if the acknowledgement sent from B is equal to the sequence number and the chekcum value is valid, the packet is positively acknowledged 
   if (packet.acknum == pktA.seqnum && packet.checksum == calculateCheckSum(packet))
   {
     stoptimer(0);
     //alter the sequence number
     seqA = 1 - seqA;
     //declare that a positive acknowledgement was recieved and that another packet can be sent
     waitingFlag = 0;
   }
...
{
  printf("A timed out\n");
  printPacketA("This packet is resent:",pktA);
  //send the packet onto the network
	tolayer3(0, pktA);
  //start the timer
	starttimer(0, TIME_OUT);
...
A_init()
{
  ...
  seqA = 0;
  //the first packet isn't waiting
  waitingFlag = 0;
}
...
{
  //if the packet recieved has the same anticipated  sequence number and wasn't corrupted
    ...
    //B sequence number will be altered to expect the next packet
    seqB = 1 - seqB;
  ...
  {
    //acknowledgement sent with the opposite sequence number
		pktB.acknum = 1- seqB;
